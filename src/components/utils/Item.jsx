...
const Item = ({ ifExists, id, color, shadow, title, text, img, btn, rating, price}) => {
  return (
    <div
        className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center justify-items-center rounded-xl py-4 px-5 trasition-all duration-700 ease-in-out w-full hover:scale-105 `}
      >
      ...
      <div className={`flex items-center ${ifExists ? 'abosulte top-5' : 'right-1 :justify-center'} `}>
        <img src={img} alt='img/item-img' className='h-36 w-64 transitions-theme hover:rotate-12' alt={`img/item-img${id}`} />
